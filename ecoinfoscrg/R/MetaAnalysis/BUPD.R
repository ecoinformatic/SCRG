...
# Parallel
# nt <- min(parallel::detectCores())
best_model <- glmmTMB(formula=best_formula, data = input, control = glmmTMBControl(parallel = 24))
...
# model <- lmer(formula, data = data)
  model <- glmmTMB(formula=best_formula, data = input, control = glmmTMBControl(parallel = 24))
...
# current_model <- lmer(current_formula, data = input)
  current_model <- glmmTMB(formula=best_formula, data = input, control = glmmTMBControl(parallel = 24))
...
final_model <- glmmTMB(formula=best_formula, data = input, control = glmmTMBControl(parallel = 24))
# summary(final_model)
# final formula
